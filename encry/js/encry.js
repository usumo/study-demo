! function () {
    ...
    /**
     * 主入口
     * 从function d中可以看出加密流程，先随机生成一个16位的密钥，将正常的请求参数与g先加密一次，再与16位的密钥加密一次，此结果为POST参数的param。
     * 由于密钥是在客户端上随机产生的，所以要将随机密钥进行一次RSA加密后作为encSecKey参数POST到服务端。
     * 所以， 整体加密方式为， 客户端生成16位密钥， 将请求参数加密两次后， 将参数加密后的结果和密钥加密后的结果一起传到服务端完成解密过程。
     * @param {string} d 正常的请求参数
     * @param {string} e RSA中加密所需的key
     * @param {string} f RSA中加密所需的modulus
     * @param {string} g AES加密的key
     */
    function d(d, e, f, g) {
        var h = {},
            i = a(16);
